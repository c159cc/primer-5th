...
template <typename T> Vec<T>::Vec(const Vec &v) {
    /**
     * @brief newData is a pair of pointers pointing to newly allocated and
     * copied from range : [b, e)
     */
...
template <typename T> Vec<T>::Vec(std::initializer_list<T> l) {
 ...
 */
template <typename T> void Vec<T>::reserve(std::size_t n) {
 ...
 */
template <typename T> void Vec<T>::resize(std::size_t n) { resize(n, T()); }
 ...
 *  given data.
 */
template <typename T> void Vec<T>::resize(std::size_t n, const T &t) {
    if (n < size()) {
 ...
 * @param b
 * @param e
 * @return  a pair of pointers pointing to [first element , one past the last)
 * in the new space
 */
template <typename T> std::pair<T *, T *> Vec<T>::alloc_n_copy(T *b, T *e) {
 ...
 */
template <typename T> void Vec<T>::wy_alloc_n_move(std::size_t n) {
...
/**
 * @brief   Double the capacity and using std::move move the original data to
...
void Vec<T>::emplace_back(Args &&... args) {
    chk_n_alloc();
    alloc.construct(first_free++, T(std::forward<Args>(args)...));
}